...
export async function syncEmailsToDatabase(emails: EmailMessage[], accountId: string) {
    ...
    try {
       await Promise.all(emails.map(email => upsertEmail(email, accountId, 0)))
    } catch (error) {
...
async function upsertEmail(email: EmailMessage, accountId: string, index: number) {
    ...
    try {
        let emailLabelTypes: 'inbox' | 'sent' | 'draft' = 'inbox'
        if (email.sysLabels.includes('inbox') || email.sysLabels.includes('important')) {
            emailLabelTypes = 'inbox'
        } else if (email.sysLabels.includes('sent')) {
            emailLabelTypes = 'sent'
        } else if (email.sysLabels.includes('draft')) {
            emailLabelTypes = 'draft'
        }
        ...
        const thread = await db.thread.upsert({
            ...
            create: {
                ...
                draftStatus: emailLabelTypes === 'draft',
                inboxStatus: emailLabelTypes === 'inbox',
                sentStatus: emailLabelTypes === 'sent',
                lastMessageDate: new Date(email.sentAt),
                participantIds: [...new Set([
                    fromAddress.id,
                    ...toAddresses.map(a => a!.id),
